	...
	<? if (CSalePaySystemAction::GetParamValue('QUOTE_EN_HEADER_SHOW') == 'Y'): ?>
        <table class="header">
            <tr><?
				...
				if ($pathToLogo) {
					...
					?>
                    <td style="padding-right: 5pt; "><img src="<?= $imgParams['SRC']; ?>" width="<?= $imgWidth; ?>"/>
                    </td><?
				}
				?>
                <td><?
					...
					if (!$sellerName)
						$sellerName = '';
					?>
                    <b><?= htmlspecialcharsbx($sellerName) ?></b><br><?
					...
					if ($sellerAddress) {
						if (is_array($sellerAddress)) {
							...
							?><?= nl2br(htmlspecialcharsbx($sellerAddress)) ?><?
						}
						unset($sellerAddress);
						?><br><?
					}
					...
					<? if (CSalePaySystemAction::GetParamValue("SELLER_PHONE", false)) { ?>
						<?= sprintf("Phone: %s", htmlspecialcharsbx(CSalePaySystemAction::GetParamValue("SELLER_PHONE", false))); ?>
                        <br>
					<? } ?>
        ...
        <div style="margin: 0pt; padding: 0pt;">
			<? if (CSalePaySystemAction::GetParamValue("BUYER_NAME", false)) { ?>
                <b><?= htmlspecialcharsbx(CSalePaySystemAction::GetParamValue("BUYER_NAME", false)); ?></b>
                <br><?
				...
				if ($buyerPayerName) {
					?><?= sprintf("Contact person: %s", htmlspecialcharsbx($buyerPayerName)); ?><br><?
				}
        ...
        <table width="100%" style="font-weight: bold">
			...
			<span style="font-size: 1.5em; font-weight: bold; text-align: center;">
				<?= sprintf('Quote # %s', htmlspecialcharsbx($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["ACCOUNT_NUMBER"])); ?>
			</span>
                </td>
                <td align="right">
	...
	if (!empty($arBasketItems)) {
		...
		foreach ($columnList as $column) {
			if (CSalePaySystemAction::GetParamValue('QUOTE_EN_COLUMN_' . $column . '_SHOW') == 'Y') {
				$arCols[$column] = array(
					'NAME' => htmlspecialcharsbx(CSalePaySystemAction::GetParamValue('QUOTE_EN_COLUMN_' . $column . '_TITLE')),
			...
			foreach ($arBasketItems as &$arBasket) {
				...
				if (is_array($arBasket['CRM_PR_FIELDS'])) {
					if (isset($arBasket['CRM_PR_FIELDS']['DISCOUNT_TYPE_ID'])
						...
						if ($arBasket['CRM_PR_FIELDS']['DISCOUNT_TYPE_ID'] === Discount::PERCENTAGE) {
							...
							$discountIsSet = true;
						} else if ($arBasket['CRM_PR_FIELDS']['DISCOUNT_TYPE_ID'] === Discount::MONETARY) {
							$discountSum = round(doubleval($arBasket['CRM_PR_FIELDS']['DISCOUNT_SUM']), 2);
