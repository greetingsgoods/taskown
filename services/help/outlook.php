...
if (CModule::IncludeModule("intranet") && CModule::IncludeModule("iblock")) {
    ...
    <table style="margin-top:7px;">
        <tr>
            <td>
                <li>
                    <a href="javascript:<? echo htmlspecialcharsbx(CIntranetUtils::GetStsSyncURL(array('LINK_URL' => '/company/'), 'contacts')) ?>"><?= GetMessage("SERVICES_LINK1") ?></a>
            </td>
        </tr>
        <tr>
            <td>
                <li>
                    <a href="javascript:<? echo htmlspecialcharsbx(CIntranetUtils::GetStsSyncURL(array('LINK_URL' => '/' . $USER->GetID() . '/'), 'tasks')) ?>"><?= GetMessage("SERVICES_LINK2") ?></a>
            </td>
        </tr>
		<?
			...
			if ($arRs = $dbRs->Fetch()):
				...
				while ($arRs2 = $dbRs2->GetNext()):
                    ...
                    <tr>
                        <td>
                            <li>
                                <a href="javascript:<? echo htmlspecialcharsbx(CIntranetUtils::GetStsSyncURL(array('ID' => $arRs2["ID"], 'LINK_URL' => 'company/personal/user/' . $USER->GetID() . '/calendar/'), 'calendar')) ?>"><?= GetMessage("SERVICES_CONNECT") ?> <?= $arRs2["NAME"] ?></a>
                        </td>
                    </tr>
				<?endwhile ?>
			<?endif ?>
