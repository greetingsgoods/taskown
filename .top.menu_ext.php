...
use Bitrix\Crm\Tracking\Manager;
...
$bLandingIncluded = Loader::includeModule("landing");
...
if (CModule::IncludeModule("crm") && CCrmPerms::IsAccessEnabled()) {
	$arMenuB24[] = array(
		...
		array(SITE_DIR . "crm/"),
		array(
			"real_link" => EntityViewSettings::getDefaultPageUrl(),
			"counter_id" => "crm_all",
...
if (CModule::IncludeModule("crm") && Manager::isAccessible()) {
	$arMenuB24[] = array(
...
if (CModule::IncludeModule("sender") && Access::current()->canViewAnything()) {
	$arMenuB24[] = array(
...
if (CModule::IncludeModule("intranet") && CIntranetUtils::IsExternalMailAvailable()) {
	$warningLink = $mailLink = Option::get('intranet', 'path_mail_client', SITE_DIR . 'mail/');
...
if (Loader::includeModule('rpa') && Driver::getInstance()->isEnabled()) {
	$arMenuB24[] = [
